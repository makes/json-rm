...
class JSONSchema:
    ...
    @classmethod
    def from_file(cls, path, normalization=False):
    ...
    def render(self, formatter, ignore_empty=False):
        return formatter.format_schema(self, ignore_empty)
    ...
    def _recurse(self, json_obj, path=[], entity=None, name_level=1, path_level=0):
        if not entity and not self._entities:
            # create root entity
            entity = Entity(path, path_level, None)
            ...
            for name, prop in json_obj.items():
                ...
                if datatype == "object":
                    ...
                    if not self._normalization:
                        ...
                        assert(entity)
                        self._recurse(prop["properties"], path=path+[name], entity=Entity(path+[name], path_level, metadata), name_level=name_level+1, path_level=path_level)
                elif datatype == "array":
                    assert(entity)
                    ...
                    array_entity = Entity(path+[name], len(path), items_metadata, enclosing_attr=array)
